...
const toggleSign = () => {
  switch (sign.value) {
    case "up":
      router.push({ query: { sign: "in" } });
      break;
    case "in":
      router.push({ query: { sign: "up" } });
      break;
  }
};
...
const handleConfirm = async () => {
  ...
  switch (sign.value) {
    case "up":
      ...
      if (user) router.push({ path: "/home/authentication/confirmation" });
      break;

    case "in":
      user = await userstore.signIn();
      if (user) router.push({ path: "/app" });
      break;
    ...
    <section class="w-full h-full grow flex justify-center">
      ...
      >
        ...
        <div class="w-full h-fit">
          <p class="text-3xl text-center text-violet-600 dark:text-violet-400">
        ...
        <div class="flex flex-col gap-3">
          ...
          <p v-if="creadentials.error" class="text-sm text-center text-red-600">
            {{
              creadentials.error
                ?.split(" ")
